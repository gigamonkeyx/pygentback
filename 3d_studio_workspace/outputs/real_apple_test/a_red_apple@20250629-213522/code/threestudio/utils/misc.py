...
try:
    ...
    # Complete mock tinycudann implementation for systems without it
    import torch
    import torch.nn as nn

    class MockEncoding(nn.Module):
        def __init__(self, n_input_dims, config, dtype=None):
            super().__init__()
            self.n_input_dims = n_input_dims
            self.config = config
            self.dtype = dtype or torch.float32
            # Set reasonable output dimensions based on config
            if isinstance(config, dict):
                self.n_output_dims = config.get('n_features_per_level', 2) * config.get('n_levels', 16)
            else:
                self.n_output_dims = 32  # Default fallback

            # Create a simple linear layer as a placeholder
            self.linear = nn.Linear(n_input_dims, self.n_output_dims, dtype=self.dtype)
    ...
    class MockNetwork(nn.Module):
        def __init__(self, n_input_dims, n_output_dims, config):
            ...
            # Create a simple MLP
            hidden_dim = config.get('n_neurons', 64) if isinstance(config, dict) else 64
            self.network = nn.Sequential(
                nn.Linear(n_input_dims, hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim, hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim, n_output_dims)
            )
    ...
    class MockTCNN:
        @staticmethod
        def Encoding(n_input_dims, config, dtype=None):
            return MockEncoding(n_input_dims, config, dtype=dtype)

        @staticmethod
        def Network(n_input_dims, n_output_dims, config):
            return MockNetwork(n_input_dims, n_output_dims, config)

        @staticmethod
        def NetworkWithInputEncoding(n_input_dims, n_output_dims, encoding_config, network_config):
            encoding = MockEncoding(n_input_dims, encoding_config)
            network = MockNetwork(encoding.n_output_dims, n_output_dims, network_config)
