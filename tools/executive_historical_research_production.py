...
class ExecutiveHistoricalResearchConductor:
    ...
    def __init__(self):
        ...
        # Initialize research orchestrator - need to check what dependencies it requires
        # For now, let's try to initialize it properly
        try:
            self.research_orchestrator = ResearchOrchestrator(self.config)
        except Exception as e:
            logger.warning(f"Could not initialize ResearchOrchestrator: {e}")
        ...
        try:
            self.historical_agent = HistoricalResearchAgent(
                config=self.config,
                capabilities=[
                    AgentCapability.RESEARCH_PLANNING,
                    AgentCapability.WEB_RESEARCH,
                    AgentCapability.ACADEMIC_ANALYSIS
                ]
            )
        except Exception as e:
            logger.warning(f"Could not initialize HistoricalResearchAgent: {e}")
    ...
    async def execute_comprehensive_research(self) -> Dict[str, Any]:
        ...
        if not self.historical_agent:
            logger.error("Historical research agent not initialized - cannot proceed")
            return {"error": "Historical research agent not initialized"}
        ...
        for topic_idx, topic in enumerate(research_topics):
            ...
            try:
                ...
                # Add to bibliography
                if "validated_sources_details" in topic_results:
                    self.results["validated_sources_bibliography"].extend(topic_results["validated_sources_details"])
    ...
    async def _execute_topic_research(self, topic: Dict[str, Any]) -> Dict[str, Any]:
        ...
        research_query = ResearchQuery(
            query_text=f"{topic['title']}: {topic['research_focus']}",
            research_type="historical_analysis",
            time_period=topic["period"].value,
            geographic_scope=topic["geographic_scope"],
            source_types=[SourceType.PRIMARY, SourceType.ARCHIVAL, SourceType.ACADEMIC],
            output_format=OutputFormat.ACADEMIC_SUMMARY,
    ...
    def _format_source_citation(self, source: Any) -> Dict[str, str]:
        ...
        return {
            ...
            "source_type": getattr(source, 'source_type', SourceType.UNKNOWN).value if hasattr(source, 'source_type') else "unknown",
