...
async def test_recipe_evolution():
    ...
    try:
        from src.evolution.advanced_recipe_evolution import AdvancedRecipeEvolution, EvolutionConfig, EvolutionStrategy, Recipe
        
        # Simple config for testing
        config = EvolutionConfig(
            population_size=10,  # Small for testing
            max_generations=3,   # Quick test
            evolution_strategy=EvolutionStrategy.GENETIC_ONLY,  # Test basic GA
        ...
        results = await evolution.evolve(
            initial_population=initial_recipes,
        ...
        if results and results.get('success'):
            ...
            for i, recipe in enumerate(best_recipes[:3]):  # Show top 3
                if isinstance(recipe, dict):
                    ...
                    print(f"  Fitness scores: {recipe.fitness_scores}")
...
async def test_distributed_genetic_algorithm():
    ...
    try:
        ...
        def test_fitness_function(genome):
            ...
            return sum(genome) if hasattr(genome, '__iter__') else float(genome)
        ...
        dga = DistributedGeneticAlgorithm(
            agent_id="test_agent",
            fitness_function=test_fitness_function,
            evolutionary_params=params
        )
        
        # Initialize the GA
        print("Initializing distributed GA...")
        await dga.initialize_distributed_ga()
        ...
        # Try to run some evolution steps
        print("Testing evolution operations...")
        
        # Test population diversity calculation
        try:
            diversity = await dga.calculate_population_diversity()
            print(f"✅ DIVERSITY CALCULATION: {diversity:.4f}")
        except Exception as e:
            print(f"❌ DIVERSITY CALCULATION FAILED: {e}")
            
        # Test population balancing
        try:
            balance_result = await dga.balance_population_load()
            print(f"✅ POPULATION BALANCING: {balance_result}")
        except Exception as e:
            print(f"❌ POPULATION BALANCING FAILED: {e}")
            
        # Test peer synchronization
        try:
            sync_result = await dga.synchronize_with_peers()
            print(f"✅ PEER SYNCHRONIZATION: {sync_result}")
        except Exception as e:
            print(f"❌ PEER SYNCHRONIZATION FAILED: {e}")
...
async def test_collaborative_self_improvement():
    ...
    try:
        from src.orchestration.collaborative_self_improvement import CollaborativeSelfImprovement
        ...
        if task.decomposition and len(task.decomposition) > 0:
            ...
            for i, subtask in enumerate(task.decomposition):
...
async def inspect_evolution_implementation():
    """Inspect what our evolution system actually does"""
    print("\n" + "=" * 60)
    print("INSPECTING EVOLUTION IMPLEMENTATION")
    print("=" * 60)
    
    print("RECIPE EVOLUTION METHODS:")
    try:
        from src.evolution.advanced_recipe_evolution import AdvancedRecipeEvolution
        methods = [method for method in dir(AdvancedRecipeEvolution) if not method.startswith('_')]
        for method in sorted(methods):
            print(f"  - {method}")
    except Exception as e:
        print(f"Could not inspect AdvancedRecipeEvolution: {e}")
    
    print("\nDISTRIBUTED GA METHODS:")
    try:
        from src.orchestration.distributed_genetic_algorithm import DistributedGeneticAlgorithm
        methods = [method for method in dir(DistributedGeneticAlgorithm) if not method.startswith('_')]
        for method in sorted(methods):
            print(f"  - {method}")
    except Exception as e:
        print(f"Could not inspect DistributedGeneticAlgorithm: {e}")
        
    print("\nCOLLABORATIVE SELF-IMPROVEMENT METHODS:")
    try:
        from src.orchestration.collaborative_self_improvement import CollaborativeSelfImprovement
        methods = [method for method in dir(CollaborativeSelfImprovement) if not method.startswith('_')]
        for method in sorted(methods):
            print(f"  - {method}")
    except Exception as e:
        print(f"Could not inspect CollaborativeSelfImprovement: {e}")
...
async def main():
    ...
    await inspect_evolution_implementation()
